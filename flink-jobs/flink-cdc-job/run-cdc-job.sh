...
JAR_FILE="$SCRIPT_DIR/build/libs/flink-cdc-job-1.0.0.jar"
MAIN_CLASS="com.flink.cdc.job.MySQLCDCJob"
...
print_banner() {
    echo -e "${BLUE}"
    echo "======================================================================"
    echo "  Flink CDC Job - MySQL to Kafka"
    echo "======================================================================"
    echo -e "${NC}"
}
...
check_prerequisites() {
    ...
    if [ ! -f "$JAR_FILE" ]; then
        ...
        print_info "먼저 빌드를 실행하세요: ./gradlew :flink-cdc-job:shadowJar"
        exit 1
    fi
...
build_jar() {
    ...
    cd "$PROJECT_ROOT"
    ./gradlew :flink-cdc-job:clean :flink-cdc-job:shadowJar
...
run_docker() {
    ...
    local container_jar="/opt/flink/jobs/flink-cdc-job/build/libs/flink-cdc-job-1.0.0.jar"
